...
class Test(TestCase):
    def test_is_message_overwatch_time(self):
        ...
        for success_test_case in success_test_cases:
            self.assertTrue(utils.is_message_overwatch_time(success_test_case))

        for failure_test_case in failure_test_cases:
            self.assertFalse(utils.is_message_overwatch_time(failure_test_case))
    ...
    def test__is_long_form_overwatch_time(self):
        ...
        failure_test_cases = ['i really want to go to the store', "rimworld", "gorilla", "king kong", "i can't a",
                              'busy', "can't tonight"]
        for success_test_case in success_test_cases:
            self.assertTrue(utils._is_long_form_overwatch_time(success_test_case))

        for failure_test_case in failure_test_cases:
            self.assertFalse(utils._is_long_form_overwatch_time(failure_test_case))
